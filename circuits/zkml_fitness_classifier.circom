...
template ZkMLFitnessClassifier(numSessions, numReadingsPerSession) {
    ...
    signal sessionInWeek[numSessions];
    component cmpAfterStart[numSessions];
    component cmpBeforeEnd[numSessions];
    ...
    for (var i = 0; i < numSessions; i++) {
        ...
        cmpAfterStart[i] = GreaterEqThan(BITS_TIMESTAMP);
        cmpAfterStart[i].in[0] <== timestamps[i][0];
        cmpAfterStart[i].in[1] <== weekTimestamp;

        cmpBeforeEnd[i] = LessThan(BITS_TIMESTAMP);
        cmpBeforeEnd[i].in[0] <== timestamps[i][numReadingsPerSession - 1];
        cmpBeforeEnd[i].in[1] <== weekTimestamp + WEEK_SECONDS;

        sessionInWeek[i] <== cmpAfterStart[i].out * cmpBeforeEnd[i].out;

        sessionConsistent[i] <== sessions[i].isConsistent * sessionInWeek[i];
